		...
		if(isset($_GET['dosubmit'])) {
			...
			foreach($_POST['ids'] as $id) {
				...
				$this->search_db->delete_search($typeid,$id);
              	$keywords_db = pc_base::load_model('keyword_model');
				$keywords_data_db = pc_base::load_model('keyword_data_model');
				$thiskey_arr = $keywords_data_db->select(array('siteid'=>$siteid,'contentid'=>$id.'-'.$modelid));
				if($thiskey_arr){
				    foreach ($thiskey_arr as $val){
				        $keywords_db->update(array('videonum'=>'-=1'),array('id'=>$val['tagid']));
                    }
				    $keywords_data_db->delete(array('siteid'=>$siteid,'contentid'=>$id.'-'.$modelid));
                }
				//删除视频库与内容对应关系数据
				if ($video_install ==1) {
