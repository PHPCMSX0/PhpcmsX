...
class manage extends admin {
	...
	public function init() {
		...
		$infos = $this->db->listinfo($where, 'aid DESC', $page, $pagesize = 30,'',10,'',array(),'aid');//
		foreach ($infos as $v) {
			if (isset($v['aid']) && !empty($v['aid'])) {
				$ids[] = $v['aid'];
			} 
		}
		unset($infos);
		if (!empty($ids)) {
			$ids = implode('\',\'', $ids);
			$infos = $this->db->select("`aid` IN ('$ids')", '*', '', '', '', '');
	...
	public function dir() {
		...
		$local = str_replace(array(PC_PATH, PHPCMS_PATH ,DS.DS), array('','',DS), $filepath);
		//$show_header = true;
	...
	public function pullic_delthumbs() {
		...
		if(!in_array(strtoupper($ext),array('JPG','GIF','BMP','PNG','JPEG'))&& stripos($filepath,$this->upload_path,0)===false)  exit('0');
