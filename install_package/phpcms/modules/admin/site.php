...
class site extends admin {
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$routeArr = pc_base::load_config('route');
			$routDomain = parse_url($domain);
			if($routDomain['path']){
				$routeKey = $routDomain['host'].$routDomain['path'];
			}else{
				$routeKey = $routDomain['host'];
			}
			$routeKey = str_replace('/','',$routeKey);
			$newSiteId = $this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style),true);
			if ($newSiteId) {
				$class_site = pc_base::load_app_class('sites');
				$class_site->set_cache();
				$routeArr[$routeKey] = array(
					'm'=>'content',
					'c'=>'index',
					'a'=>'init',
					'data'=>array('GET'=>array('siteid'=>intval($newSiteId)))
				);
				$routeConfigStr = config2str($routeArr);
				set_route($routeConfigStr);
	...
	public function del() {
		...
		$objSite = $this->db->get_one(array('siteid'=>$siteid));
		if ($objSite) {
			$objDomain = $objSite['domain'];
			$routDomain = parse_url($objDomain);
			if($routDomain['path']){
				$routeKey = $routDomain['host'].$routDomain['path'];
			}else{
				$routeKey = $routDomain['host'];
			...
			if ($this->db->delete(array('siteid'=>$siteid))) {
				...
				foreach ($routeArr as $key => $val){
					if($key != $routeKey) $newConfigArr[$key] = $val;
				}
				$routeConfigStr = config2str($newConfigArr);
				set_route($routeConfigStr);
