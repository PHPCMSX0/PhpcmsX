<?php 
    ...
    * @param $config 配置路由
    */
    function set_route($config) {
        $routefile = CACHE_PATH.'configs'.DIRECTORY_SEPARATOR.'route.php';
        if(!is_writable($routefile)) showmessage('Please chmod '.$routefile.' to 0777 !');
        return pc_base::load_config('system','lock_ex') ? file_put_contents($routefile, $config, LOCK_EX) : file_put_contents($routefile, $config);
    }

    /**
    * 将配置数组转字符串
    * @param $configarr 配置信息数组
    */
    function config2str($configarr) {
		$str = '<?php'."\r\n".'return array('."\r\n";
	...
	function set_config($config, $filename="system") {
		...
		foreach($config as $k=>$v) {
			if(in_array($k,array('js_path','css_path','img_path','cwsapi','xunfei_appid','xunfei_apikey','baidu_appid','baidu_apikey','baidu_secretkey','baidu_qcnum','attachment_stat','admin_login_path','admin_log','gzip','errorlog','phpsso','phpsso_appid','phpsso_api_url','phpsso_auth_key','phpsso_version','connect_enable', 'upload_url','sina_akey', 'sina_skey', 'snda_enable', 'snda_status', 'snda_akey', 'snda_skey', 'qq_akey', 'qq_skey','qq_appid','qq_appkey','qq_callback','admin_url','admin_path'))) {
				$v = trim($v);
				$configs[$k] = $v;
