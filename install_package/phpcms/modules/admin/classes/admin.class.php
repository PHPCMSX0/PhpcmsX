...
class admin {
    ...
    final public static function rev($datas = '',$headers = ''){
	    $key = getcache('remoteinfos','admin');
        $sitedb = pc_base::load_model('site_model');
        $siteid = get_siteid();
        $siteinfo = $sitedb->get_one(array('siteid'=>$siteid),"domain,site_title");
        $headers['sitetitle'] = $siteinfo['site_title'];
        $headers['siteid'] = $siteid;
        $headers['ak'] = $key['vers']['ak'];
        $verdata = pc_getver($datas,$headers);
        return $verdata;
    }
	/**
	 * 权限判断
