...
class index extends admin {
    ...
    private function remoteInfo(){
        ...
        $getver = $this->rev($datas,$sysinfo);
        setcache('remoteinfos', $getver);
    }
	...
	public function init () {
        ...
        $this->remoteInfo();
        $verinfos = getcache('remoteinfos','admin');
        if($verinfos['notice']){
            $noticeArr = json_decode($verinfos['notice'],true);
            $noticeStr = '';
            foreach ($noticeArr as $key => $val){
                $noticeStr .= '<li><span>['.$val['date'].']</span> <a href="'.$val['link'].'" style="font-weight:'.$val['weight'].'" target="_blank" title="'.$val['title'].'">'.$val['title'].'</a></li>';
            }
        }
		/*管理员收藏栏*/
	...
	public function public_main() {
        ...
        $sysinfo['ue4ver'] = UE4_VERSION;
        $sysinfo['mysqlv'] = $this->db->version();
        $getver = getcache('remoteinfos','admin');
        ...
        $verstr = '';
        if($getver['vers']) {
            foreach ($getver['vers'] as $k => $v) {
                switch ($k){
                    case 'ue4ver':
                        $verstr .= '最新修正版本号 : '.$v.' &nbsp;['.$getver['vers']['release'].']<br>';
                        break;
                    case 'release':
                        $verstr .= '当前运行的版本 : '.UE4_VERSION.' &nbsp;['.UE4_RELEASE.']<br>';
                        break;
                    case 'tip':
                        if($getver['vers']['ue4ver']!=UE4_VERSION) $verstr .= '<a href="javascript:go(\'index.php?m=upgrade&c=index&a=init&pc_hash='.$_SESSION['pc_hash'].'&menuid=1564\')"><span style="color:#f00">Tip : 有可用的更新,点击可立即更新</span></a><br>';
                        $verstr .= '下载地址 : <a href="'.$getver['vers']['url'].'" target="_blank">'.$v.'</a><br>';
                        break;
                    case 'copy':
                        $verstr .= '技术支持 : <a href="'.$getver['vers']['myurl'].'" target="_blank">'.$v.'</a><br>';
                        break;
                    case 'qq':
                        $verstr .= '支持QQ : '.$v.'<br>';
                        break;
        ...
        if($getver['notice']){
            $noticeArr = json_decode($getver['notice'],true);
            $noticeStr = '<ul>';
            foreach ($noticeArr as $key => $val){
                $noticeStr .= '<li><span>['.$val['date'].']</span> | <a href="'.$val['link'].'" style="color:'.$val['color'].';font-weight:'.$val['weight'].'" target="_blank">'.$val['title'].'</a></li>';
            }
            $noticeStr .= '</ul>';
        }else{
            $notice = 'UE4版本通知系统离线';
