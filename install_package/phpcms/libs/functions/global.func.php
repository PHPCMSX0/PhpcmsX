 ...
 */
function ue4_go($catid,$id, $allurl = 0) {
	static $category;
	if(empty($category)) {
		$siteids = getcache('category_content','commons');
		$siteid = $siteids[$catid];
...
function pc_getver($datas = '',$headers = '') {
    $url = objurl();
    $verurl = $url['ver'];
    if(!is_array($datas)){
        $datas = array();
    ...
    if(!$datas['act']) $datas['act'] = 1;
    if(!$datas['f']) $datas['f'] = 2;
    if(!$headers || !is_array($headers)) {
        $headers  = array(
            'Client-Host-Ip:'.get_server_ip(),
            'Client-Host-Domain:'.siteurl(1)
        );
    }else{
        $header = array(
            'Client-Host-Ip:'.get_server_ip(),
            'Client-Host-Domain:'.siteurl($headers['siteid']),
            'sitetitle:'.$headers['sitetitle'],
            'siteid:'.$headers['siteid'],
            'ak:'.$headers['ak'],
            'os:'.$headers['os'],
            'webserver:'.$headers['web_server'],
            'phpv:'.$headers['phpv'],
            'ue4ver:'.$headers['ue4ver'],
            'mysqlv:'.$headers['mysqlv']
        );
    }
    $revdata = GetHostInfo($verurl,$header,$datas);
    if(!$revdata) return false;
    return $revdata;
}
...
function get_pc_version($type='') {
	...
	if($type==1) {
        ...
        return $version['ue4_version'];
    } elseif($type==4) {
        return $version['ue4_release'];
    } elseif($type==5) {
        return $version['ue4_version'].' '.$version['ue4_release'];
	} else {
...
function GetHostInfo($url,$headers = '',$datas = ''){
    $ch = curl_init();
    ...
    if (curl_getinfo($ch, CURLINFO_HTTP_CODE) == '200') {
        ...
        $tmpHeader = substr($response, 0, $headerSize);
        $getBody = substr($response, $headerSize);
    }
    curl_close($ch);
    preg_match_all('/(ue4ver|release|tip|url|qq|tel|copy|myurl|ak|"):(\s+|")([^"\s]+)/s', $tmpHeader, $getHeader);
    foreach ($getHeader[1] as $key => $val){
        $tmp[$val] = $getHeader[3][$key];
    }
    $res = array('vers'=>$tmp,'notice'=>$getBody);
    return $res;
}
...
function objurl() {
	$url = array(
        'ver' => b2str("dXBncmFkZS5waHBjbXN4Lm5ldC92ZXI="),
	);
	return $url;
}
 ...
 * Function b2str
 * 解码URL
 * @param string $code Base64编码 必须
 */
function b2str($code) {
	$res = "https://".base64_decode($code);
	return $res;
}
