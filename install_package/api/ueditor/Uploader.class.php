...
class Uploader
{
    ...
    public function __construct($fileField, $config, $type = "upload")
    {
        ...
        $catarr = $cat_db->get_one(array('catid'=>$this->catid),'siteid');
        $this->siteid = intval($catarr['siteid'])== 0 ? 1 : intval($catarr['siteid']);
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            if($this->siteid) {
                if (is_file(CACHE_PATH.'caches_commons/caches_data/sitelist.cache.php')) {
                    $site_setting = require CACHE_PATH.'caches_commons/caches_data/sitelist.cache.php';
                }
                $site_setting = json_decode($site_setting[1]['setting'], true);
            ...
            $image = new image(1,1);
            if($watermark_enable) {
                $image->watermark($this->filePath,$this->filePath);
            }
        }
        // 存储附件
        $this->save_attach();
    ...
    private function save_attach()
    {
        ...
        $uploadedfile['filepath'] = str_replace('/'.$this->getMyPath($this->fullName).'/','',$this->fullName);
     ...
     * @return string
     */
    private function getMyPath($fullname){
        $upload_url = explode('/',pc_base::load_config('system','upload_url'));
        $upload_url = array_filter($upload_url,function($v){return !empty($v);});
        $fullname = explode('/',$fullname);
        $fullname = array_filter($fullname,function($v){return !empty($v);});
        $theSame = array_intersect($upload_url,$fullname);
        return implode('/',$theSame);
    }
