...
class index extends admin {
	...
	public function right() {
		...
		foreach($mysql_table_status as $table) {
			//艺术流氓增 修复PHP7.1+ 警告
			$mysql_table_size = intval($mysql_table_size);
			$mysql_table_index_size = intval($mysql_table_index_size);
			$table['Data_length'] = intval($table['Data_length']);
			$table['Index_length'] = intval($table['Index_length']);
