...
switch($step)
{
	...
	case '2':  //环境检测 (FTP帐号设置）
        ...
        $PHP_DNS = preg_match("/^[0-9.]{7,15}$/", @gethostbyname('www.phpcmsx.com')) ? 1 : 0;
		//是否满足phpcms安装需求
	...
	case 'installmodule': //执行SQL
		...
		if($module == 'admin') {
			...
			if(file_exists(PHPCMS_PATH."install/main/".$dbfile)) {
				...
				//删除原后台登录地址
				@unlink(PHPCMS_PATH."admin.php");
				//建立自定义后台登录目录
				if(file_exists(PHPCMS_PATH.$adminpath)) {
					file_put_contents(PHPCMS_PATH.$adminpath."/index.php","<?php header('location:../index.php?m=admin&a=".$adminpath."');?>");
				}else{
					mkdir(PHPCMS_PATH.$adminpath, 0775, true);
					file_put_contents(PHPCMS_PATH.$adminpath."/index.php","<?php header('location:../index.php?m=admin&a=".$adminpath."');?>");
				}

				$pmai = file_get_contents(PHPCMS_PATH.'phpcms/modules/admin/index.php');
				...
				file_put_contents(PHPCMS_PATH."phpcms/modules/admin/index.php",$pmai);

				$pmaca = file_get_contents(PHPCMS_PATH.'phpcms/modules/admin/classes/admin.class.php');
			...
			showmessage(L('admin_login'),'?m=admin&c=index&a=".$adminpath."'",$pmaca);
				file_put_contents(PHPCMS_PATH."phpcms/modules/admin/classes/admin.class.php",$pmaca);

				$pmatl = file_get_contents(PHPCMS_PATH.'phpcms/modules/admin/templates/login.tpl.php');
				$pmatl = str_replace("m=admin&c=index&a=login","m=admin&c=index&a=".$adminpath,$pmatl);
				file_put_contents(PHPCMS_PATH."phpcms/modules/admin/templates/login.tpl.php",$pmatl);

				$pmath = file_get_contents(PHPCMS_PATH.'phpcms/modules/admin/templates/header.tpl.php');
				$pmath = str_replace("m=admin&c=index&a=admin","m=admin&c=index&a=".$adminpath,$pmath);
				$pmath = file_put_contents(PHPCMS_PATH."phpcms/modules/admin/templates/header.tpl.php",$pmath);
